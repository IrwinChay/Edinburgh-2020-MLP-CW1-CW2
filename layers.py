...
class LeakyReluLayer(Layer):
    ...
    def bprop(self, inputs, outputs, grads_wrt_outputs):
        ...
        gradients with respect to the layer inputs.
        """
        return np.where( outputs > 0, grads_wrt_outputs, self.alpha * grads_wrt_outputs) 
...
class RandomReluLayer(Layer):
    ...
    def fprop(self, inputs, leakiness=None):
        ...
        if (leakiness is None):
            leakiness = self.rng.uniform(self.lower, self.upper, inputs.size)
            leakiness = leakiness.reshape(inputs.shape)
        
        self.leakiness = leakiness
        
        return np.where(inputs > 0, inputs, leakiness * inputs)
    ...
    def bprop(self, inputs, outputs, grads_wrt_outputs):
        ...
        gradients with respect to the layer inputs.
        """
        return np.where( outputs > 0, grads_wrt_outputs, self.leakiness * grads_wrt_outputs) 
...
class ParametricReluLayer(LayerWithParameters):
    ...
    def fprop(self, inputs):
        ...
        y = x                          if x > 0
          = alpha * x                  otherwise
        """
        return np.where(inputs > 0, inputs, self.params[0] * inputs)
    ...
    def grads_wrt_params(self, inputs, grads_wrt_outputs):
        ...
        """
        grad_of_output_wrt_alpha = np.where( inputs > 0, 0, inputs )
        mulp = grads_wrt_outputs * grad_of_output_wrt_alpha
        mulp = mulp.sum()
        return [ mulp ]
...
class ExponentialLinearUnitLayer(Layer):
    ...
    def fprop(self, inputs):
        ...
        """
        return np.where( inputs >= 0, inputs, self.alpha * ( np.power(e, inputs) - 1) )
    ...
    def bprop(self, inputs, outputs, grads_wrt_outputs):
        ...
        gradients with respect to the layer inputs.
        """
        return np.where( outputs > 0, grads_wrt_outputs, self.alpha * np.power(e, inputs) * grads_wrt_outputs)
